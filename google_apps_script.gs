function doGet(e) {
  ...
  if (tipo === 'evolutivo_pereciveis') {
    return ContentService
      .createTextOutput(JSON.stringify(readEvolutivoPereciveis(ss)))
      .setMimeType(ContentService.MimeType.JSON);
  }
...
function readEvolutivoPereciveis(ss) {
  var nomes = ['Evolutivo_Pereciveis', 'evolutivo_pereciveis', 'Evolutivo pereciveis'];
  var sheet = null;
  for (var i = 0; i < nomes.length; i++) {
    sheet = ss.getSheetByName(nomes[i]);
    if (sheet) {
      break;
    }
  }
  if (!sheet) {
    return { error: 'Aba "Evolutivo_Pereciveis" não encontrada.' };
  }

  var lastRow = sheet.getLastRow();
  if (lastRow < 1) {
    return { error: 'Aba "Evolutivo_Pereciveis" vazia.' };
  }

  var values = sheet.getRange(1, 1, lastRow, 4).getDisplayValues();
  return values.map(function(row) {
    return { ano: row[0], mes: row[1], setor: row[2], quebra: row[3] };
  }).filter(function(item) {
    return item.ano || item.mes || item.setor || item.quebra;
  });
}